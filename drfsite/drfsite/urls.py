...
urlpatterns = [
    ...
    # ---23.12.2023---
    # Lesson 11
    # В Django 5 logout ПРОИЗВОДИТСЯ ТОЛЬКО ПО POST-ЗАПРОСУ, ПОЭТОМУ ПЕРЕОПРЕДЕЛЯЕМ МАРШРУТ ДЛЯ logout
    path('api/v1/drf-auth/logout/', logout_user, name='logout'),

    # Авторизацию и аутентификацию в DRF (также с помощью сторонних пакетов) можно выполнять несколькими способами
    # Рассмотрим авторизацию на основе сессия (session-based) в DRF
    # Название маршрута придумываем сами, также подключаем маршруты urls из rest_framework
